...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    var cameraVCDelegate: CameraVCDelegate? = nil
...
//		cameraButton.isEnabled = false
        self.cameraVCDelegate?.shouldEnableCameraUI(enabled: false)
//		recordButton.isEnabled = false
        self.cameraVCDelegate?.shouldEnableRecordUI(enabled: false)
//		photoButton.isEnabled = false
        self.cameraVCDelegate?.shouldEnablePhotoButton(enabled: false)
//		livePhotoModeButton.isEnabled = false
        self.cameraVCDelegate?.shouldEnableLivePhotoModeButton(enabled: false)
//		captureModeControl.isEnabled = false
        self.cameraVCDelegate?.shouldEnabledCaptureModeControl(enabled: false)
    ...
    internal weak var _previewView: PreviewView!
...
//	@IBAction private func toggleCaptureMode(_ captureModeControl: UISegmentedControl) {
    func toggleCaptureMode() {
        ...
        if self.cameraVCDelegate?.chosenSegmentedControlIndex() == CaptureMode.photo.rawValue {
...
//						self.livePhotoModeButton.isEnabled = true
                        self.cameraVCDelegate?.shouldEnableLivePhotoModeButton(enabled: true)
//						self.livePhotoModeButton.isHidden = false
                        self.cameraVCDelegate?.livePhotoModeButtonHidden(hidden: false)
					}
				}
			}
		}
//		else if captureModeControl.selectedSegmentIndex == CaptureMode.movie.rawValue
        else if self.cameraVCDelegate?.chosenSegmentedControlIndex() == CaptureMode.movie.rawValue
		{
...
//						self.recordButton.isEnabled = true
                        self.cameraVCDelegate?.shouldEnableRecordUI(enabled: true)
					}
				}
			}
		}
...
//				self.cameraButton.isEnabled = true
                self.cameraVCDelegate?.shouldEnableCameraUI(enabled: true)
//				self.recordButton.isEnabled = self.movieFileOutput != nil
                self.cameraVCDelegate?.shouldEnableRecordUI(enabled: self.movieFileOutput != nil)
//				self.photoButton.isEnabled = true
                self.cameraVCDelegate?.shouldEnablePhotoButton(enabled: true)
//				self.livePhotoModeButton.isEnabled = true
                self.cameraVCDelegate?.shouldEnableLivePhotoModeButton(enabled: true)
//				self.captureModeControl.isEnabled = true
                self.cameraVCDelegate?.shouldEnabledCaptureModeControl(enabled: true)
			}
		}
	}
...
//			self.recordButton.setTitle(NSLocalizedString("Stop", comment: "Recording button stop title"), for: [])
            self.cameraVCDelegate?.shouldEnableRecordUI(enabled: true)
            self.cameraVCDelegate?.recordingHasStarted()
		}
	}
    ...
    func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
		...
		*/
		...
		if error != nil {
            ...
            self.cameraVCDelegate?.videoRecordingFailed()
		}
		
		if success {
            self.cameraVCDelegate?.videoRecordingComplete(videoUrl: outputFileURL)
            /*
...
//			self.cameraButton.isEnabled = self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1
            self.cameraVCDelegate?.shouldEnableCameraUI(enabled: self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1)
//			self.recordButton.isEnabled = true
            self.cameraVCDelegate?.shouldEnableRecordUI(enabled: true)
//			self.captureModeControl.isEnabled = true
            self.cameraVCDelegate?.shouldEnabledCaptureModeControl(enabled: true)
//			self.recordButton.setTitle(NSLocalizedString("Record", comment: "Recording button record title"), for: [])
            self.cameraVCDelegate?.canStartRecording()
		}
	}
...
//				self.cameraButton.isEnabled = isSessionRunning && self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1
                self.cameraVCDelegate?.shouldEnableCameraUI(enabled: isSessionRunning && self.videoDeviceDiscoverySession.uniqueDevicePositionsCount() > 1)
//				self.recordButton.isEnabled = isSessionRunning && self.movieFileOutput != nil
                self.cameraVCDelegate?.shouldEnableRecordUI(enabled: isSessionRunning && self.movieFileOutput != nil)
//				self.photoButton.isEnabled = isSessionRunning
                self.cameraVCDelegate?.shouldEnablePhotoButton(enabled: isSessionRunning)
//				self.captureModeControl.isEnabled = isSessionRunning
                self.cameraVCDelegate?.shouldEnabledCaptureModeControl(enabled: isSessionRunning)
//				self.livePhotoModeButton.isEnabled = isSessionRunning && isLivePhotoCaptureEnabled
                self.cameraVCDelegate?.shouldEnableLivePhotoModeButton(enabled: isSessionRunning && isLivePhotoCaptureEnabled)
//				self.livePhotoModeButton.isHidden = !(isSessionRunning && isLivePhotoCaptureSupported)
                self.cameraVCDelegate?.livePhotoModeButtonHidden(hidden: !(isSessionRunning && isLivePhotoCaptureSupported))
			}
		}
		else {
